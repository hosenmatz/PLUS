...
// Modified by Chen ChingWen 2017/7/13    
//   SecondButton, ThirdButton  USE internal pullup resistors
// Interrupt   FALLING
...
const char programID[] = "OttoAN_v2"; //Each program will have a ID
...
void setup() {
  ...
  //Serial communication initialization 設定 串列傳輸速度 baudrate
  Serial.begin(57600);

  pinMode(PIN_SecondButton, INPUT_PULLUP); //使用內建上拉電阻 Pull-up resistors
  ...
  //Interrumptions
  enableInterrupt(PIN_SecondButton, secondButtonPushed, FALLING);// 中斷 當訊號下降時觸發（FALLING）
  enableInterrupt(PIN_ThirdButton, thirdButtonPushed, FALLING);

  //Setup callbacks for SerialCommand commands 串列控制指令
  SCmd.addCommand("S", receiveStop);      //  sendAck & sendFinalAck 停止
  SCmd.addCommand("L", receiveLED);       //  sendAck & sendFinalAck LED點矩陣 顯示 L 000000001000010100100011000000000
  SCmd.addCommand("T", recieveBuzzer);    //  sendAck & sendFinalAck 輸入蜂鳴器頻率與節拍 T 1000 500
  SCmd.addCommand("M", receiveMovement);  //  sendAck & sendFinalAck 動作  M 1, M 2,  ... M 20
  SCmd.addCommand("H", receiveGesture);   //  sendAck & sendFinalAck LED表情與動作 H 1 ... H 12
  SCmd.addCommand("K", receiveSing);      //  sendAck & sendFinalAck  內建聲音    K 1  ... K 16
  SCmd.addCommand("C", receiveTrims);     //  sendAck & sendFinalAck  輸入 Sevro 修正值 C  5 5 10 10
  SCmd.addCommand("G", receiveServo);     //  sendAck & sendFinalAck  Sevro 角度 G 80 100 80 100
  SCmd.addCommand("R", receiveName);      //  sendAck & sendFinalAck  輸入 機器人的名字
  SCmd.addCommand("E", requestName);      //  讀取 機器人名字
  SCmd.addCommand("D", requestDistance);  // 讀取 超音波距離
  SCmd.addCommand("N", requestNoise);     // 讀取 音量值
  SCmd.addCommand("B", requestBattery);   // 讀取 電池電壓
  SCmd.addCommand("I", requestProgramId); // 讀取 程式版本
  SCmd.addDefaultHandler(receiveStop);    // 預設指令(停止 STOP)
